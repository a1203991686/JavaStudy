    ...
    public void run() {
        try {
            while(flag) {
                ...
                if(message.getType()==MessageType.TYPE_SEND&&!(message.getFrom().equals(name))) {
                    System.out.println("[" + message.getFrom() + "]" + "说:" + message.getInfo());
                }else if(message.getType()==MessageType.TYPE_SEND&&(message.getFrom().equals(name))) {
                    System.out.println("我说:"+message.getInfo());
                }else if (message.getType()==MessageType.TYPE_EXIT){
                    System.out.println("["+message.getFrom()+"]"+message.getInfo());
                } else if (message.getType()==MessageType.TYPE_LOGIN) {
                    System.out.println("["+message.getFrom()+"]"+message.getInfo());
                }
            }
        } catch (IOException | ClassNotFoundException e) {
